...
func (s *ConnectServiceHandler) GetMemoInsight(ctx context.Context, req *connect.Request[v1pb.GetMemoInsightRequest]) (*connect.Response[v1pb.GetMemoInsightResponse], error) {
	resp, err := s.APIV1Service.GetMemoInsight(ctx, req.Msg)
	if err != nil {
		return nil, convertGRPCError(err)
	}
	return connect.NewResponse(resp), nil
